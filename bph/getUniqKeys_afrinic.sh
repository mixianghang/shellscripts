...
while [ $day -le $endDate ]
  do
	if [ ! -f ${curDir}/$day/afrinic.db ]
	then
	  gzip -d ${curDir}/$day/afrinic.db.gz
	fi
	inetnum=$(grep -E -c "^inet[6]?num:" ${curDir}/$day/afrinic.db)
	grep -E "(admin-c|tech-c):[ ]+\w+" ${curDir}/$day/afrinic.db | sed -E 's/(tech-c|admin-c):[ ]+([1-9a-zA-Z\-]+)/\2/g' | sort -u > uniqKeysFrom_$day
